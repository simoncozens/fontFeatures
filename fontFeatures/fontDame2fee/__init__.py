...
def add_to_feature_table(parsed, line):
  ...
  for l in m[3].split(", "):
    ...
    if not l in parsed["lookups"]:
      parsed["lookups"][l] = { "feature": parsed["features"][-1]["tag"], "languages": [] }
...
def get_class(parsed, cid, lookup):
  ...
  if len(res) > 5:
    ...
    if classname in parsed["config"]: return parsed["config"][classname]
    return classname
...
def add_to_lookup(parsed, line):
  ...
  if m:
    if m[2] == "yes": lookup["flags"].append(m[1])
    return
...
def add_to_class_definition(parsed, line):
  ...
  if not m: print(line)
  if not "classes" in lookup: lookup["classes"] = {"0":[]}
  if not m[2] in lookup["classes"]: lookup["classes"][m[2]] = []
...
def apply_transformations(rule,parsed):
  ...
  for ix,c in reversed(list(enumerate(context))):
    if ix in transformations:
      ...
      if lookup["type"] == "single"  and isinstance(context[ix],str) and not(context[ix].startswith("@")):
        for r in lookup["rules"]:
          if context[ix] == r[0]: context[ix] = r[1]
      elif lookup["type"] == "multiple" and isinstance(context[ix],str)  and not(context[ix].startswith("@")):
        for r in lookup["rules"]:
          if context[ix] == r[0]:
            if ix < len(context):
        ...
        if lookupname in parsed["config"]: lookupname = parsed["config"][lookupname]
...
def unparse(filename, config=None):
  ...
  def unparse_lookup_rules(lookup):
    for r in lookup["rules"]:
      ...
      if "languages" in lookup and len(lookup["languages"]) != len(parsed["all_languages"]):
  ...
  for lid in sorted(parsed["lookups"].keys(), key=int):
    lookup = parsed["lookups"][lid]
    if not("feature" in lookup): continue
