...
class NewYB:
    ...
    def action(self, parser, w):
        ...
        for bariye in parser.fontfeatures.namedClasses["bariye"]:
            ...
            dropYBsRoutine = fontFeatures.Routine(flags= 0x0010)
            dropYBsRoutine.markFilteringSet = below_dots

            dropADotRoutine = fontFeatures.Routine()
            # Substitute those number ending .yb with those ending .yb
            below_dots_non_yb = list(sorted(filter(lambda x:not x.endswith(".yb"), below_dots)))
            below_dots_yb = list(sorted(filter(lambda x:x.endswith(".yb"), below_dots)))
            if len(below_dots_non_yb) != len(below_dots_yb):
                raise ValueError("Mismatch in @below_dots: %s has .yb suffix, %s does not" % (below_dots_yb, below_dots_non_yb))

            dropADotRoutine.addRule(fontFeatures.Substitution([below_dots_non_yb], [below_dots_yb]))
            ...
            while len(queue) > 0:
                ...
                # format 2 class-based rule! Wonderful!
                    dropYBsRoutine.addRule(chainrule)
            ...
            routines.append(dropYBsRoutine)
        return routines
