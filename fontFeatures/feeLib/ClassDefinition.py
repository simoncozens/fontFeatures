...
class DefineClass:
  ...
  def validate(self, tokens, verbaddress):
    ...
    if tokens[1].token != "=":
      raise ParseError("Expected something to equal something else", tokens[1].address, self)

    if tokens[2].token.startswith("/"):
      if not tokens[-1].token.endswith("/"):
        raise ParseError("Unterminated regular expression", tokens[2].address, self)
  ...
  def expandRegex(self, parser, tokens):
    ...
    try:
      ...
      raise ParseError("Couldn't parse regular expression", tokens[2].address, self)
...
class ShowClass():
  ...
  def store(self, parser, tokens):
    name = tokens[0].token
    print("Expanding class %s -> [%s]" % (name, " ".join(parser.expandGlyphOrClassName(name))))
    return []