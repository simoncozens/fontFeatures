...
class MergeNonOverlappingRoutines:
  ...
  def apply(self, ff):
    ...
    def haschains(routine):
      for r in routine.rules:
        if isinstance(r, Chaining): return True

    routinelist = ff.routines
    ff.markRoutineUseInChains()
    for lix, l in enumerate(routinelist):
      if haschains(l) or l == deadRoutine: continue
      for rix in range(lix+1, len(routinelist)):
        r = routinelist[rix]
        if haschains(r) or r == deadRoutine: continue
        if self.nonOverlapping(ff, l,r):
          logger.info("Merging nonoverlapping routines %s , %s" % (l.name, r.name))
          self.merge(l,r)
    ...
    for i in usedin:
      for routinelist in i.lookups:
        ...
        for ix,routine in enumerate(routinelist):
          if routine == first: routinelist[ix] = second
  ...
  def nonOverlapping(self, ff, l, r):
    return not(l.involved_glyphs & r.involved_glyphs) \
          and not(self.whatuses(l,ff) & self.whatuses(r,ff)) \
          and self.compatibleRules(l,r)

  def compatibleRules(self, l, r):
    from fontFeatures.feaLib.Routine import arrange_by_type,arrange_by_lookup_type,arrange_by_flags
    testRoutine = Routine()
    testRoutine.rules.extend(l.rules)
    testRoutine.rules.extend(r.rules)
    if arrange_by_type(testRoutine) : return False
    if arrange_by_lookup_type(testRoutine): return False
    if arrange_by_flags(testRoutine): return False
    return True

optimizations = [
  MergeNonOverlappingRoutines
]