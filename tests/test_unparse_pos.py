...
class TestUnparse(unittest.TestCase):
  ...
  def test_single_f1(self):
    g,_ = self.unparser.unparseLookup(self.lookups[1]) # part of kern
    self.assertEqual(g.rules[0].asFea(),"pos uni0621.float <-146 0 0 0>;")

  def test_single_f1_class(self):
    g,_ = self.unparser.unparseLookup(self.lookups[3]) # part of kern
  ...
  def test_single_f2(self):
    g,_ = self.unparser.unparseLookup(self.lookups[3]) # lookups called in mark feature
  ...
  def test_pair_f1(self):
    g,_ = self.unparser.unparseLookup(self.lookups[56]) # proportional digits in kern feature
