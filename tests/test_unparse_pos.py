...
class TestUnparse(unittest.TestCase):
  ...
  def test_single_f1_class(self):
    ...
    self.assertEqual(g.rules[0].asFea(),"pos [uni0627.fina uni0627.fina_KafMemAlf uni0627.fina_MemAlfFina uni0627.fina_Wide] <122 0 122 0>;")

  def test_single_f2(self):
    g,_ = self.unparser.unparseLookup(self.lookups[3], 3) # lookups called in mark feature
  ...
  def test_pair_f1(self):
    g,_ = self.unparser.unparseLookup(self.lookups[56], 56) # proportional digits in kern feature
  ...
  def test_pair_f2(self):
    ...
    self.assertEqual(g.rules[0].asFea(),"pos [zero zero.prop] [A Aacute Abreve Acircumflex Adieresis Agrave Amacron Aogonek Aring Atilde] -10;")
    self.assertEqual(g.rules[1].asFea(),"pos [zero zero.prop] [Y Yacute Ycircumflex Ydieresis Ygrave] -21;")
