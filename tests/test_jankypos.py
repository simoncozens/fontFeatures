...
class TestPositioning(unittest.TestCase):
  ...
  def test_urdu(self):
    ...
    self.assertEqual(buf[0][1].asFea(), ValueRecord(xAdvance=750).asFea())
    self.assertEqual(buf[1][0], "SeenMed")
    self.assertEqual(buf[1][1].asFea(), ValueRecord(xAdvance=539, xPlacement = -1, yPlacement = 335).asFea())
    self.assertEqual(buf[2][0], "SeenIni")
    self.assertEqual(buf[2][1].asFea(), ValueRecord(xAdvance=607, xPlacement = -2, yPlacement = 558).asFea())
